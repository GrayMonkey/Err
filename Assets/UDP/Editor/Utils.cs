...
namespace UnityEngine.UDP.Editor
{
    public static class Utils
    {
       ...
       public static string RetrieveClientIdFromLocalFile(string settingPath)
       {
           var ret = "";
           if (File.Exists(settingPath))
           {
               Func<string, bool> notCommentMsg = s => !s.Contains("*");
               IEnumerable<string> contents = File.ReadAllLines(settingPath);
               foreach (var line in contents.Where(notCommentMsg))
               {
                   ret = line;
               }
           }
